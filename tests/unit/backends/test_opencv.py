"""Tests for the OpenCV-backed frame extraction backend."""
...
def test_is_available_returns_true_in_this_environment() -> None:
    assert OpenCVBackend.is_available() is True
...
def test_frame_shape_and_dtype(synthetic_video_path):
    ...
    assert first.color_space == "bgr"
    assert first.source_backend == "opencv"
